	...
	if ( wpdm_query_var( 'sociallogin' ) == 'facebook' ) {
		...
		if ( ! isset( $_GET['code'] ) && ! isset( $_GET['access_token'] ) ) {
			...
			if ( intval( $user_id ) > 0 ) {
				...
				if ( (int) $user_id <= 0 || user_can( $user_id, 'manage_options' ) ) {
					return;
				}
...
function ConnectHelper() {
	...
	if ( intval( $user_id ) > 0 ) {
		...
		\WPDM\__\Email::send( "user-signup", array(
			'to_email' => $user_email,
			'name'     => $display_name,
			'username' => $user_login,
			'password' => $user_pass
            ...
            <div id="wpdmslb-facebooklike-<?php echo $pid; ?>">
                <div class="labell">
                    <script>
                        ...
                        window.fbAsyncInit = function () {
                            ...
                            FB.Event.subscribe('edge.create', function (href) {
                                ...
                                jQuery.post("<?php echo wpdm_rest_url( 'facebook-liked' ); ?>", {
                                    __wpdm_ID:<?php echo $pid; ?>,
                    ...
                    <div class="fb-like" data-layout="standard" data-action="like" data-size="large"
                         data-show-faces="false" data-href="<?php echo $url; ?>" data-send="false" data-width="300"
                         data-font="arial">L o a d i n g . . .
                    </div>

                    <style>.fb_edge_widget_with_comment {
