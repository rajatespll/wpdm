...
class Asset
{
    ...
    function newLink($access = array('roles' => array('guest'), 'users' => array())){
        global $wpdb;
        $asset_key = uniqid();
        $wpdb->insert("{$wpdb->prefix}ahm_asset_links", array('asset_ID' => $this->ID, 'asset_key' => $asset_key, 'access' => json_encode($access)));
        $this->links = $this->getLinks();
        return $this;
    }
    ...
    public static function getLink($ID){
        ...
        $link->url = home_url('/wpdm-asset/'.$link->asset_key);
        $link->access = json_decode($link->access, true);
    ...
    public static function preview($path){
        ...
        $accessible = !$accessible || strstr($accessible[0], '403') ? false : true;
    ...
    public static function view($path){
        global $current_user;
        $ext = WPDM()->fileSystem::fileExt($path);
        $url = str_replace(ABSPATH, home_url('/'), $path);
        $accessible = ini_get('allow_url_fopen') && __::is_url($url) ? @get_headers($url) : [403];
        $accessible = !$accessible || substr_count($accessible[0], '403') ? false : true;
        $relpath = str_replace(AssetManager::root(), "", $path);
        ...
        if($ext == 'pdf') {
	        return "<iframe src='{$asset->temp_download_url}&play=1' style='width: 100%;height: calc(100vh - 73px);border: 0;margin: 0'></iframe>";
        }
        if(in_array($ext, ['ppt', 'pptx', 'doc', 'docx'])) {
	        return "<iframe src='https://view.officeapps.live.com/op/view.aspx?src={$asset->temp_download_url}&amp;play=1' style='width: 100%;height: calc(100vh - 73px);border: 0;margin: 0'></iframe>";
        }
