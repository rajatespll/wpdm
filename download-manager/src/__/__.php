		...
		if (is_array($value))
			...
			switch ($sanitize) {
				...
				case 'kses':
					...
					/*$allowedtags['div'] = array('class' => true, 'style' => true);
					$allowedtags['strong'] = array('class' => true);
					$allowedtags['p'] = array('class' => true, 'style' => true);
	...
	static function isAuthentic($nonce_var, $nonce_key, $access_level, $is_ajax = true){
		...
		if($is_ajax) {
			if(!check_ajax_referer($nonce_key, $nonce_var, false))
				wp_send_json(['success' => false, 'type' => 'error', 'message' => esc_attr__( 'Referer verification failed', WPDM_TEXT_DOMAIN )]);
		}
		if(!wp_verify_nonce(wpdm_query_var($nonce_var), $nonce_key)) wp_send_json(['success' => false, 'type' => 'error', 'message' => __('Security token is expired! Refresh the page and try again.', 'download-manager')]);
		if(!current_user_can($access_level)) wp_send_json(['success' => false, 'type' => 'error', 'message' => __( "You are not allowed to execute this action!", "download-manager" )]);
	}
