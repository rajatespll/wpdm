...
class __MailUI {
	...
	static function panel( $heading = '', $content = array(), $footer = false ) {
	...
	{
		return '<div class="uibox" style="padding: 20px; background: #ffffff; border: 1px solid #d7dcea;border-radius: 4px;margin: 10px 0">' . $content . '</div>';
	}
	...
	static function button( $link, $label, $bg = '#0085d8', $padding = '10px 20px', $radius = 4, $width = 'block', $margin = '10px 0') {
		$link   = esc_attr( $link );
		$button = "<a href='{$link}' style='background: {$bg}; color: #ffffff; border-radius: {$radius}px;display: {$width}; padding: {$padding};margin: {$margin};text-align: center;text-decoration: none'>{$label}</a>";
		return $button;
	}
