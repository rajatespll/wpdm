...
?><form method="post" id="edit_profile" name="edit-profile" action="" class="form">
    <div class="card card-default dashboard-card mb-3">
        <div class="card-header">
            <a target="_blank" href="<?php echo get_option('__wpdm_author_profile') > 0?get_permalink(get_option('__wpdm_author_profile'))."?user=".$current_user->user_nicename:get_author_posts_url($current_user->ID);?>" class="btn btn-info btn-xs pull-right" style="margin: 0;"><?php _e( "View Profile" , "download-manager" ); ?></a>
