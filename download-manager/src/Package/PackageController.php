...
class PackageController extends PackageTemplate {
	...
	function prepare( $ID = null, $template = null, $template_type = 'page' ) {
		...
		$post_vars['description'] = stripcslashes( str_replace("[wpdm", "[__wpdm", $post_vars['post_content']) );
		...
		if ( in_array( 'file_download_links', $template_tags ) ) {
			$post_vars['file_download_links']    = FileList::fileDownloadLinks( $ID );
		}
		...
		if ( array_intersect( array( 'file_type_icon', 'icon', 'icon_url' ), $template_tags ) ) {
			$post_vars['files'] = isset( $post_vars['files'] ) && is_array( $post_vars['files'] ) ? $post_vars['files'] : $this->getFiles( $ID );
			...
			$post_vars['icon_url'] = self::icon($ID);
	 ...
	 * @return string
	 */
	public static function videoPlayer( $ID, $files = null, $width = 800, $height = 600 ) {
		...
		foreach ( $files as $index => $file ) {
			if ( __::is_url( $file ) && ( substr_count( $file, 'vimeo.com' ) || substr_count( $file, 'youtube.com' )  || substr_count( $file, 'wistia' ) ) ) {
				$videos[] = $file;
				$vidx[]   = $index;
			} else {
				$realpath = file_exists( $file ) ? $file : UPLOAD_DIR . $file;
		...
		if ( count( $videos ) > 0 ) {
			...
			if ( __::is_url( $videos[0] ) ) {
				if(substr_count($videos[0], 'vimeo'))
					$player_html = self::embedVimeo($videos[0], $width, $height);
				else if(substr_count($videos[0], 'wistia'))
					$player_html = self::embedWistia($videos[0], $width, $height);
				else if(substr_count($videos[0], 'youtu'))
					$player_html = self::embedYouTube($videos[0], $width, $height);
				else {
					$video = FileSystem::mediaURL( $ID, $ind, wpdm_basename( $videos[0] ) );
					$player_html = "<video id='__wpdm_videoplayer' class='card p-3' width=\"{$width}\" controls controlsList='nodownload'><source src=\"{$video}\" type=\"video/mp4\">Your browser does not support HTML5 video.</video><div class='videothumbs'>{$videothumbs}</div>";
				}
			} else {
				$video = FileSystem::mediaURL( $ID, $ind, wpdm_basename( $videos[0] ) );
				$player_html = "<video id='__wpdm_videoplayer' class='card p-3' width=\"{$width}\" controls controlsList='nodownload'><source src=\"{$video}\" type=\"video/mp4\">Your browser does not support HTML5 video.</video><div class='videothumbs'>{$videothumbs}</div>";
		...
		if(WPDM()->package->isExpired($ID)) $player_html = UI::card(\__('Package Expired', WPDM_TEXT_DOMAIN), [sprintf(\__('This package was expired on %s', 'download-manager'), date_i18n( get_option( 'date_format' ) . " h:i A", (int)strtotime( get_post_meta( $ID, '__wpdm_expire_date', true ) ) + wpdm_tzoffset() ))]);
		return $player_html;
	}
	...
	function isExpired($ID)
	{
		...
		if(strtotime($expired) < time()) return true;
		return false;
	}
...
//                        $adata .= '<div class="modal fade" role="modal" id="pkg_' . $package['ID'] . "_" . $unqid . '"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><strong style="margin:0px;font-size:12pt">' . __('Download') . '</strong></div><div class="modal-body">' . $data . '</div><div class="modal-footer text-right"><button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Close</button></div></div></div></div>';
	...
	function getLinkLabel( $ID = null, $template_type = 'link' ) {
		...
		$link_label = ( trim( $link_label ) === '' ) ? __( "Download", "download-manager" ) : __::sanitize_var($link_label, 'kses');
	...
	public function downloadLink( $ID, $embed = 0, $extras = [] ) {
		...
		if ( $expired !== "" && strtotime( $expired ) < time() ) {
			...
			return "<button type='button' title='{$msg}' onclick='alert(\"{".esc_attr($msg)."}\");' class='{$style} btn-danger ttip'>{$link_label}</div>";//$link_label = __("Download was expired on", "download-manager") . " " . date_i18n(get_option('date_format') . " h:i A", strtotime($expired));
		...
		//User doesn't have permission to download
	...
	function expirableDownloadLink( $ID, $usageLimit = 10, $expirePeriod = 10800, $sessionOnly = true ) {
		...
		if ( ! $sessionOnly ) {
			...
			TempStorage::set( "__wpdmkey_{$key}_{$ID}", $exp, $expirePeriod );
		}
		$permalink    = get_permalink( $ID );
		$permalink    = apply_filters( "wpdm_download_url_base", $permalink, $ID );
	...
	/**
	 * Generate custom download page url
	 * @param $ID
	 * @param int $usageLimit
	 * @param int $expirePeriod seconds | default 10800 seconds = 3 hours
	 *
	 * @return string
	 */
	static function expirableDownloadPage( $ID, $usageLimit = 10, $expirePeriod = 10800, $sessionOnly = true ) {
		if(!$ID || get_post_type($ID) !== 'wpdmpro') return false;
		$key = uniqid();
	...
	public function fetchTemplate( $template, $vars, $type = 'link' ) {
		...
		$popen    = preg_quote( '[protected]' );
		$pclose   = preg_quote( '[/protected]' );
		if(! self::userCanAccess( $vars['ID'] )) {
			$_template = preg_filter( "~$popen(.*)$pclose~misU", "", $template );
			$template = $_template ? $_template : $template;
			$_template = preg_filter( "/\[protected_msg=([^\]]+)\]/", "<div class='alert alert-danger'>$1</div>", $template );
			$template = $_template ? $_template : $template;
		} else {
			$_template = preg_filter( "~$popen(.*)$pclose~misU", "$1", $template );
			$template = $_template ? $_template : $template;
			$_template = preg_filter( "/\[protected_msg=([^\]]+)\]/", "", $template );
			$template = $_template ? $_template : $template;
		}
		...
		preg_match_all( "/\[video_player_([0-9]+)x([0-9]+)\]/", $template, $vdmatches );
		...
		foreach ( $vdmatches[0] as $nd => $scode ) {
			...
			$vars[ $scode ] = self::videoPlayer( $vars['ID'], $vars['files'], $vdmatches[1][ $nd ] , $vdmatches[2][ $nd ] );
		}
	...
	public function docPreview( $package ) {
		...
		if ( $this->userCanAccess( $package['ID'] ) || (int) get_option( '__wpdm_public_doc_preview' ) ) {
			if ( strpos( $ifile, "://" ) ) {
				$url = $ifile;
			} else {
				$file = reset( $files );
				if ( ! substr_count( strtolower( $file ), '.pdf' ) || !class_exists('\WPDM\AddOn\PDFViwer') ) {
					$url            = $this->expirableDownloadLink( $package['ID'], 3, 600, false );
					$params['open'] = 1;
			...
			if ( is_user_logged_in() ) {
				$doc_preview_html = UI::card( esc_html__( 'Document preview failed', WPDM_TEXT_DOMAIN ), [ Messages::permission_denied( $package['ID'] ) ], '', [ 'style' => 'margin-bottom: 15px' ] );
			} else {
				$doc_preview_html = UI::card( esc_html__( 'Document preview failed', WPDM_TEXT_DOMAIN ), [ Messages::login_required( $package['ID'] ) ], '', [ 'style' => 'margin-bottom: 15px' ] );
			}
		}
	...
	function addViewCount() {
		...
		__::isAuthentic( '__wpdm_view_count', NONCE_KEY, 'read', true );
	...
	static function embedYouTube($url, $width, $height)
	{
		if ( strpos( $url, 'youtu' ) ) {
			...
			if ( isset( $vid ) ) {
				return "<iframe width=\"{$width}\" height=\"{$height}\" src=\"https://www.youtube.com/embed/{$vid}\" frameborder=\"0\" allowfullscreen></iframe>";
			}
		}
		return '';
	}

	static function embedVimeo($url, $width, $height)
	{

		$res = __::remote_get("https://vimeo.com/api/oembed.json?url={$url}&width={$width}&height={$height}");
		$res = json_decode($res);
		return $res->html;

	}

	static function embedWistia($url, $width, $height)
	{

		$url = add_query_arg(['controlsVisibleOnLoad' => 'true'], $url);
		return "<iframe width=\"{$width}\" height=\"{$height}\" src=\"{$url}\" frameborder=\"0\" allowtransparency=\"true\" scrolling=\"no\" class=\"wistia_embed\" name=\"wistia_embed\" allowfullscreen mozallowfullscreen webkitallowfullscreen oallowfullscreen msallowfullscreen></iframe><script src=\"//fast.wistia.net/assets/external/iframe-api-v1.js\"></script>";

	}

	static function embedLocalVideo($video, $width, $videothumbs = '')
	{

		return "<video id='__wpdm_videoplayer' class='card p-3' width=\"{$width}\" controls controlsList='nodownload'><source src=\"{$video}\" type=\"video/mp4\">Your browser does not support HTML5 video.</video><div class='videothumbs'>{$videothumbs}</div>";
