...
$items_per_page = __::valueof($params, 'items_per_page', 20);
                ...
                if (WPDM()->package->userCanAccess($data['ID']) || !get_option("_wpdm_hide_all", 0)) {
                    ...
                    <tr class="__dt_row">
                        ...
                        foreach ($cols as $colx => $cold) {
                            $dor = array('publish_date' => strtotime(get_the_date('Y-m-d')), 'date' => strtotime(get_the_date('Y-m-d')), 'create_date' => strtotime(get_the_date('Y-m-d')), 'update_date' => strtotime(get_the_modified_date('Y-m-d', get_the_ID())), 'modified' => strtotime(get_the_modified_date('Y-m-d', get_the_ID())));
                            ?>
                            <td <?php if (in_array($cold[0], array('publish_date', 'date', 'update_date', 'modified', 'create_date'))) { ?> data-order="<?php echo $dor[$cold[0]]; ?>" <?php } ?>
