...
function wpdm_download_file( $filepath, $filename, $speed = 0, $resume_support = 1, $extras = array() ) {
	...
	if ( ! file_exists( $filepath ) ) {
		Messages::fullPage( "Download Error", "<div class='card bg-danger text-white text-left' style='min-width: 300px'><div class='card-header'>" . __( "Download Error", "download-manager" ) . "</div><div class='card-body'>" . __( "File Not Found!", "download-manager" ) . "</div></div>" );
	}
	if ( isset( $_GET['play'] ) ) {
		$extras['play'] = $_GET['play'];
	}
...
function wpdm_file_size( $file ) {
	return WPDM()->fileSystem->fileSize( $file );
}
...
function wpdm_post_thumb( $size = '', $echo = true, $extra = null ) {
	...
	if ( is_array( $size ) ) {
		...
		if ( $large_image_url == '' && isset( $extra['default'] ) ) {
			$large_image_url = $extra['default'];
		}
		if ( $large_image_url != '' ) {
...
function wpdm_thumb( $post, $size = '', $echo = true, $extra = null ) {
	...
	if ( is_array( $size ) ) {
		...
		if ( $large_image_url != '' ) {
			...
			if ( $alt === '' ) {
				$alt = esc_attr( strip_tags( get_the_title( $post->ID ) ) );
			}
        ...
        <div id="wpdm-drag-drop-area">
            ...
            <div class="panel-footer">
				...
				$plupload_init = array(
					...
					'filters'             => array(
						array(
							'title'      => __( 'Allowed Files', WPDM_TEXT_DOMAIN ),
...
function wpdm_option_field( $data ) {
	...
	switch ( $data['type'] ):
		...
		case 'radio':
			...
			break;
		case 'notice':
			return "<div class='well' style='margin: 0;box-shadow: none'>$data[notice]</div>" . $desc;
		case 'textarea':
...
function wpdm_user_dashboard_url( $params = array() ) {
	...
	$url = apply_filters( "wpdm_user_dashboard_url", $url, $params );
...
function wpdm_admin_tpl_path( $file, $tpldir = '', $fallback = '' ) {
	$admin_tpl_paths = [
		'',
		get_stylesheet_directory() . '/download-manager/admin/',
		get_template_directory() . '/download-manager/admin/',
	];
	if ( $tpldir !== '' ) {
		$admin_tpl_paths[] = $tpldir;
	}

	$admin_tpl_paths[] = WPDM_BASE_DIR . "src/Admin/views/";

	if ( $fallback !== '' ) {
		$admin_tpl_paths[] = $fallback;
	}

	$admin_tpl_paths = apply_filters( "wpdm_admin_tpl_path", $admin_tpl_paths );
	$path            = '';
    foreach ( $admin_tpl_paths as $admin_tpl_path ) {
	    if ( file_exists( realpath(trailingslashit( $admin_tpl_path ) . $file) ) ) {
		    $path = realpath( trailingslashit( $admin_tpl_path ) . $file );
		    break;
	    }
    }
	...
	/*if ( file_exists( get_stylesheet_directory() . '/download-manager/admin/' . $file ) ) {
		...
		$path = WPDM_BASE_DIR . "src/Admin/views/" . $file;
	}
...
function wpdm_paginate_links( $total, $items_per_page, $current_page = 1, $var = 'cp', $params = array() ) {
	...
	//wpdmprecho($pags);
	$phtml = "";
	$rel   = wpdm_valueof( $params, 'rel' );
	if ( is_array( $pags ) ) {
		foreach ( $pags as $pagl ) {
			$phtml .= "<li>{$pagl}</li>";
		}
		if ( isset( $params['container'] ) ) {
			$phtml = str_replace( "<a", "<a {$rel} data-container='{$params['container']}'", $phtml );
		}
	}
...
add_action( "admin_head", function () {
	...
	if ( wpdm_query_var( 'post_type' ) !== 'wpdmpro' || ! in_array( wpdm_query_var( 'page' ), $pages ) ) {
		return;
	}
	?>
    <style>
